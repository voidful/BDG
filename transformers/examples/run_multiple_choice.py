    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            if preds is None:
                ...
                entropys = np.append(entropys, entropy.detach().cpu().numpy(), axis=0)
        ...
        for i in range(len(max_pred)):
            entropy_result[inputjson[i]['id']].append([entropys[i], inputjson[i]])
        ...
        for i, (p, t) in enumerate(zip(max_pred, out_label_ids)):
            error['errors'].append([entropys[i], "predict:" + ABCD[p], "answer:" + ABCD[t], inputjson[i]])
        error['errors'] = sorted(error['errors'], key=lambda l: l[0], reverse=True)
        ...
        output_eval_file = os.path.join(eval_output_dir, "datadir_" + args.data_dir.replace("/", "").replace(".",
                                                                                                             "") + "_eval_results.txt")
        output_entropy_file = os.path.join(eval_output_dir, "datadir_" + args.data_dir.replace("/", "").replace(".",
                                                                                                                "") + "_entropy.jsonl")
